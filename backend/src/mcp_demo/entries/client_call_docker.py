...
async def _run_client(
    *, host: str, port: int, server_mount_path: str, transport: str
    ...
    client: Client = Client(
        ...
        transport=get_mcp_config_docker(  # type: ignore
            host=host,
            port=port,
    ...
    async with client:
        ...
        greet_result = await client.call_tool("greet", {"name": "Foobar"})
        ...
        try:
            await client.call_tool("foobar_tool", {"a": 5, "b": 10})
        except ToolError as e:
        ...
        try:
            await client.call_tool("add_positives_only", {"a": -5, "b": 10})
        except ToolError as e:
