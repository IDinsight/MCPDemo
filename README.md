...
1. [OPTIONAL] If you ran dev testing first, then from the root directory, run `make down-dev` to stop all dev testing containers.
2. From the root directory, run `make up-local`. This will initialize the Docker containers for local testing.
...
1. [OPTIONAL] If you ran local testing first, then from the root directory, run `make down-local` to stop all local testing containers.
